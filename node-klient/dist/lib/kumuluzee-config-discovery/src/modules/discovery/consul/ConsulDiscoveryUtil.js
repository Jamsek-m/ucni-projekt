...
import { URL } from 'url';
import { ConfigurationUtil } from 'modules/configuration';
import InitializationUtils from 'modules/discovery/common/InitializationUtils';
import ConsulServiceConfiguration from 'modules/discovery/consul/ConsulServiceConfiguration';
import ConsulRegistrator from 'modules/discovery/consul/ConsulRegistrator';
import CommonUtil from 'modules/discovery/common/CommonUtil';
import { getConsulServiceKey, getInstanceFromServiceHealth } from 'modules/discovery/consul/ConsulUtils';
  ...
  async getServiceInstances(serviceName, version, environment, accessType) {
    ...
    if (version) {
      ...
      if (accessType === 'GATEWAY' && urlList.length > 0) {
        let gatewayUrl = await this.getGatewayUrl(serviceName, resolvedVersion, environment);
        try {
