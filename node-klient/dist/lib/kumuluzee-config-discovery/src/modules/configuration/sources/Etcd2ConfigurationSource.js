...
import { ConfigurationUtil } from 'modules/configuration/utils/ConfigurationUtil';
import InitializationUtils from 'modules/configuration/utils/InitializationUtils';
...
class Etcd2ConfigurationSource {
  ...
  async init(configurationDispatcher: any) {
    ...
    const etcdUsername = await this.configurationUtil.get('kumuluzee.discovery.etcd.username') || null;
    const etcdPassword = await this.configurationUtil.get('kumuluzee.discovery.etcd.password') || null;

    // Get CA certificate
    let cert = await this.configurationUtil.get('kumuluzee.discovery.etcd.ca') || null;
    ...
    // Get etcd host names
    const etcdUrls = await this.configurationUtil.get('kumuluzee.discovery.etcd.hosts') || null;
